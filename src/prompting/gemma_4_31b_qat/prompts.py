...
class GetCodeChangeCommandsPrompt(IGetCodeChangeCommandsPrompt):
    '''Implementation of IGetCodeChangeCommandsPrompt using Gemma 4 31B QAT.'''
    _conf: Gemma431bQatConfiguration
    ...
    def execute(self, context: GetCodeChangeCommandsPromptContext) -> Result[List[CodeCommand]]:
        try:
            self._logger.debug(f"Calling Gemma 4 31B QAT API for code change commands with {len(context.code_file_paths)} code files")
            ...
            self._logger.error(f"Gemma 4 31B QAT API call failed: {e}")
            return Result.err(f"Gemma 4 31B QAT API call failed: {e}")


@dataclasses.dataclass(frozen=True)
class GetCodeChangeCommandsReprompt(IGetCodeChangeCommandsReprompt):
    '''Implementation of IGetCodeChangeReprompt using Gemma 4 31B QAT.'''
    _conf: Gemma431bQatConfiguration
    _client: openai.OpenAI = dataclasses.field(init=False)
    ...
    def execute(self, context: GetCodeChangeCommandsRepromptContext) -> Result[List[CodeCommand]]:
        try:
            self._logger.debug(f"Calling Gemma 4 31B QAT API for code change reprompt with {len(context.code_file_paths)} code files")
...
class GetCodeFixCommandsPrompt(IGetCodeFixCommandsPrompt):
    '''Implementation of IGetCodeFixCommandsPrompt using Gemma 4 31B QAT.'''
    _conf: Gemma431bQatConfiguration
    _client: openai.OpenAI = dataclasses.field(init=False)
    _logger: logging.Logger = dataclasses.field(default=logging.getLogger(__name__))
    ...
    def execute(self, context: GetCodeFixCommandsPromptContext) -> Result[List[CodeCommand]]:
        try:
            self._logger.debug(f"Calling Gemma 4 31B QAT API for code fix prompt with {len(context.code_file_paths)} code files")
