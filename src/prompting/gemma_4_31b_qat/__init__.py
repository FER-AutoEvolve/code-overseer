...
from prompting.gemma_4_31b_qat.prompts import GetCodeChangeCommandsPrompt, GetCodeChangeCommandsReprompt, GetCodeFixCommandsPrompt
...
class PromptManager(BasePromptManager):
    '''Manages prompts to the Gemma 4 31B QAT API.'''
    _configuration: Gemma431bQatConfiguration = dataclasses.field(init=False)
    _logger: logging.Logger = dataclasses.field(default=logging.getLogger())

    def __post_init__(self):
        object.__setattr__(self, '_configuration', Gemma431bQatConfiguration.from_dict(self._prompting_configuration.provider_config).unwrap())
