...
from prompting.qwen_coder_30b.prompts import GetCodeChangeCommandsPrompt, GetCodeChangeCommandsReprompt, GetCodeFixCommandsPrompt
import logging
import openai
...
class PromptManager(BasePromptManager):
    ...
    def execute_code_fix_prompt(self, strategic_description: str, error_description: str, code_file_paths: Optional[List[str]]) -> Result[List[CodeCommand]]:
        ...
        prompt = GetCodeFixCommandsPrompt(self._qwen_configuration, self._logger)
        return prompt.execute(prompt_context)
