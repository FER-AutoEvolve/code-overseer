...
from prompting.gemma_4_26b_a4b_qat.prompts import GetCodeChangeCommandsPrompt, GetCodeChangeCommandsReprompt, GetCodeFixCommandsPrompt
...
class PromptManager(BasePromptManager):
    '''Manages prompts to the Gemma 4 26B A4B QAT API.'''
    _configuration: Gemma426bA4bQatConfiguration = dataclasses.field(init=False)
    _logger: logging.Logger = dataclasses.field(default=logging.getLogger())

    def __post_init__(self):
        object.__setattr__(self, '_configuration', Gemma426bA4bQatConfiguration.from_dict(self._prompting_configuration.provider_config).unwrap())
