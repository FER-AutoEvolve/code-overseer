...
import openai
from prompting.gemma_4_26b_a4b_qat.configuration import Gemma426bA4bQatConfiguration
...
def _extract_response_text(response: object) -> Result[str]:
    ...
    for item in getattr(response, "output", []) or []:
        for content_item in getattr(item, "content", []) or []:
            ...
            text_object = getattr(content_item, "text", None)
            nested_value = getattr(text_object, "value", None)
...
class GetCodeChangeCommandsPrompt(IGetCodeChangeCommandsPrompt):
    '''Implementation of IGetCodeChangeCommandsPrompt using Gemma 4 26B A4B QAT.'''
    _conf: Gemma426bA4bQatConfiguration
    ...
    def execute(self, context: GetCodeChangeCommandsPromptContext) -> Result[List[CodeCommand]]:
        try:
            self._logger.debug(f"Calling Gemma 4 26B A4B QAT API for code change commands with {len(context.code_file_paths)} code files")
            ...
            log_token_usage(self._logger, response, "Gemma 4 26B A4B QAT")
            ...
            self._logger.error(f"Gemma 4 26B A4B QAT API call failed: {e}")
            return Result.err(f"Gemma 4 26B A4B QAT API call failed: {e}")


@dataclasses.dataclass(frozen=True)
class GetCodeChangeCommandsReprompt(IGetCodeChangeCommandsReprompt):
    '''Implementation of IGetCodeChangeReprompt using Gemma 4 26B A4B QAT.'''
    _conf: Gemma426bA4bQatConfiguration
    _client: openai.OpenAI = dataclasses.field(init=False)
    ...
    def execute(self, context: GetCodeChangeCommandsRepromptContext) -> Result[List[CodeCommand]]:
        try:
            self._logger.debug(f"Calling Gemma 4 26B A4B QAT API for code change reprompt with {len(context.code_file_paths)} code files")
...
class GetCodeFixCommandsPrompt(IGetCodeFixCommandsPrompt):
    '''Implementation of IGetCodeFixCommandsPrompt using Gemma 4 26B A4B QAT.'''
    _conf: Gemma426bA4bQatConfiguration
    _client: openai.OpenAI = dataclasses.field(init=False)
    _logger: logging.Logger = dataclasses.field(default=logging.getLogger(__name__))
    ...
    def execute(self, context: GetCodeFixCommandsPromptContext) -> Result[List[CodeCommand]]:
        try:
            self._logger.debug(f"Calling Gemma 4 26B A4B QAT API for code fix prompt with {len(context.code_file_paths)} code files")
