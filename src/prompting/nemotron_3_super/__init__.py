...
from prompting.nemotron_3_super.prompts import GetCodeChangeCommandsPrompt, GetCodeChangeCommandsReprompt, GetCodeFixCommandsPrompt


@dataclasses.dataclass(frozen=True)
class PromptManager(BasePromptManager):
    '''Manages prompts to the Nemotron 3 Super API.'''
    _configuration: Nemotron3SuperConfiguration = dataclasses.field(init=False)
    _logger: logging.Logger = dataclasses.field(default=logging.getLogger())

    def __post_init__(self):
        object.__setattr__(self, '_configuration', Nemotron3SuperConfiguration.from_dict(self._prompting_configuration.provider_config).unwrap())
