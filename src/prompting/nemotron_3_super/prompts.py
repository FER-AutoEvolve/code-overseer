...
class GetCodeChangeCommandsPrompt(IGetCodeChangeCommandsPrompt):
    '''Implementation of IGetCodeChangeCommandsPrompt using Nemotron 3 Super.'''
    _conf: Nemotron3SuperConfiguration
    ...
    def execute(self, context: GetCodeChangeCommandsPromptContext) -> Result[List[CodeCommand]]:
        try:
            self._logger.debug(f"Calling Nemotron 3 Super API for code change commands with {len(context.code_file_paths)} code files")
            ...
            self._logger.error(f"Nemotron 3 Super API call failed: {e}")
            return Result.err(f"Nemotron 3 Super API call failed: {e}")


@dataclasses.dataclass(frozen=True)
class GetCodeChangeCommandsReprompt(IGetCodeChangeCommandsReprompt):
    '''Implementation of IGetCodeChangeReprompt using Nemotron 3 Super.'''
    _conf: Nemotron3SuperConfiguration
    _client: openai.OpenAI = dataclasses.field(init=False)
    ...
    def execute(self, context: GetCodeChangeCommandsRepromptContext) -> Result[List[CodeCommand]]:
        try:
            self._logger.debug(f"Calling Nemotron 3 Super API for code change reprompt with {len(context.code_file_paths)} code files")
...
class GetCodeFixCommandsPrompt(IGetCodeFixCommandsPrompt):
    '''Implementation of IGetCodeFixCommandsPrompt using Nemotron 3 Super.'''
    _conf: Nemotron3SuperConfiguration
    _client: openai.OpenAI = dataclasses.field(init=False)
    _logger: logging.Logger = dataclasses.field(default=logging.getLogger(__name__))
    ...
    def execute(self, context: GetCodeFixCommandsPromptContext) -> Result[List[CodeCommand]]:
        try:
            self._logger.debug(f"Calling Nemotron 3 Super API for code fix prompt with {len(context.code_file_paths)} code files")
