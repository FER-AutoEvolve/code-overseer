...
from prompting.gpt_oss_20b.prompts import GetCodeChangeCommandsPrompt, GetCodeChangeCommandsReprompt, GetCodeFixCommandsPrompt
import logging
import openai
...
class PromptManager(BasePromptManager):
    '''
    Manages prompts to the GPT OSS 20B API.
    Attributes:
        _gpt_oss_configuration (GptOss20bConfiguration): Configuration for GPT OSS 20B API access.
        _logger (logging.Logger): Logger for logging information and errors.
    '''
    _gpt_oss_configuration: GptOss20bConfiguration = dataclasses.field(init=False)
    _logger: logging.Logger = dataclasses.field(default=logging.getLogger())

    def __post_init__(self):
        object.__setattr__(self, '_gpt_oss_configuration', GptOss20bConfiguration.from_dict(self._prompting_configuration.provider_config).unwrap())
    ...
    def execute_code_fix_prompt(self, strategic_description: str, error_description: str, code_file_paths: Optional[List[str]]) -> Result[List[CodeCommand]]:
        ...
        prompt = GetCodeFixCommandsPrompt(self._gpt_oss_configuration, self._logger)
        return prompt.execute(prompt_context)
