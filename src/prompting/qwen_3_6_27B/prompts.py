...
class GetCodeChangeCommandsPrompt(IGetCodeChangeCommandsPrompt):
    '''Implementation of IGetCodeChangeCommandsPrompt using Qwen 3.6 27B.'''
    _conf: Qwen3627bConfiguration
    ...
    def execute(self, context: GetCodeChangeCommandsPromptContext) -> Result[List[CodeCommand]]:
        try:
            self._logger.debug(f"Calling Qwen 3.6 27B API for code change commands with {len(context.code_file_paths)} code files")
            ...
            log_token_usage(self._logger, response, "Qwen 3.6 27B")
            ...
            self._logger.debug("Qwen 3.6 27B API call successful, parsing response")
            ...
            self._logger.error(f"Qwen 3.6 27B API call failed: {e}")
            return Result.err(f"Qwen 3.6 27B API call failed: {e}")


@dataclasses.dataclass(frozen=True)
class GetCodeChangeCommandsReprompt(IGetCodeChangeCommandsReprompt):
    '''Implementation of IGetCodeChangeReprompt using Qwen 3.6 27B.'''
    _conf: Qwen3627bConfiguration
    _client: openai.OpenAI = dataclasses.field(init=False)
    ...
    def execute(self, context: GetCodeChangeCommandsRepromptContext) -> Result[List[CodeCommand]]:
        try:
            self._logger.debug(f"Calling Qwen 3.6 27B API for code change reprompt with {len(context.code_file_paths)} code files")
...
class GetCodeFixCommandsPrompt(IGetCodeFixCommandsPrompt):
    '''Implementation of IGetCodeFixCommandsPrompt using Qwen 3.6 27B.'''
    _conf: Qwen3627bConfiguration
    _client: openai.OpenAI = dataclasses.field(init=False)
    _logger: logging.Logger = dataclasses.field(default=logging.getLogger(__name__))
    ...
    def execute(self, context: GetCodeFixCommandsPromptContext) -> Result[List[CodeCommand]]:
        try:
            self._logger.debug(f"Calling Qwen 3.6 27B API for code fix prompt with {len(context.code_file_paths)} code files")
