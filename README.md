...
Set `Prompting.Provider` to `openai` to use any OpenAI model. This provider requires a non-empty `Prompting.ProviderConfig.Model` value, for example `gpt-4.1-mini`. The existing `openai_gpt_4_1`, `openai_gpt_5`, `openai_gpt_5_mini`, and `openai_gpt_5_nano` providers remain available and keep their pinned models.
